...
namespace pikarychev_i_gauss3_vertical_mpi {
...
class TaskPar : public ppc::core::Task {
 public:
  explicit TaskPar(std::shared_ptr<ppc::core::TaskData> taskData_) : Task(std::move(taskData_)) {}
  bool pre_processing() override;
  bool validation() override;
  bool run() override;
  bool post_processing() override;
 ...
 private:
  static int distr(const boost::mpi::communicator& com, int total, std::vector<int>& sizes);
  ...
  static constexpr size_t padding = 2 * pad;
};
